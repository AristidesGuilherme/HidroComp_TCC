...
class Pearson3(DistributionBiuld):
    ...
    def cumulative(self):
        ...
        bandxaxis = go.layout.XAxis(title="Duração")
        bandyaxis = go.layout.YAxis(title="Probabilidade")

        layout = dict(title="Pearson III - Acumulada: %s" % self.title,
                      showlegend=True,
    ...
    def density(self):
        ...
        layout = dict(title="Pearson III - Densidade: %s" % self.title,
                      showlegend=True,
                      #width=945, height=827,
                      xaxis=bandxaxis,
                      yaxis=bandyaxis,
    ...
    def _data_density(self):
        ...
        density = stat.pearson3.pdf(cumulative['peaks'].values, self.shape,
    ...
    def _data_cumulative(self):
        ...
        quantiles = stat.pearson3.ppf(probability, self.shape,
                                        loc=self.location,
                                        scale=self.scale)
